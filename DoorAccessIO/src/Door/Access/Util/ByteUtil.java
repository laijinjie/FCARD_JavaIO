...
public class ByteUtil {
     ...
     * @return
     */
    public static ByteBuf BCDToBytes(ByteBuf buf, int iIndex, int iLen)
    {
        while (iLen > 0)
        {
    ...
    private static final long UINT32MAX = 4294967295L;
    private static final long UINT8MAX = 255L;
    ...
    public static long CreateCRC32(byte data[], int off, int len) {
        long iDigest = UINT32MAX, order = 32L;
        long tmp2, bits = 8L;
        int size = off + len;
        for (int i = off; i < size; i++) {
            tmp2 = ((iDigest >> (order - bits)) & UINT8MAX);
            iDigest = (iDigest << bits) ^ CRCTable[(int) ((tmp2 ^ data[i]) & 0xff)];
        }
        return iDigest & UINT32MAX;
    }
     ...
     */
    public static void writeString(ByteBuf buf, String value,int len) throws UnsupportedEncodingException {
        byte[] bt = new byte[len];
        byte[] btValue = value.getBytes(StandardCharsets.UTF_16BE);
     ...
     * @throws UnsupportedEncodingException
     */
    public static String getString(ByteBuf buf, int len) throws UnsupportedEncodingException {
        byte[] bt = new byte[len];
        buf.readBytes(bt, 0, len);
        ...
        int iBSize=byteList.size();
        byte[] bytes = new byte[iBSize];
